module.exports.run = async (bot, message, args) => {
    ...
    if(message.member.voiceChannelID !== message.guild.members.get(bot.user.id).voiceChannelID) return error.invalid(message, "Skip", "You must in the channel with the bot to skip!")
              ...
              if (guilds[message.guild.id].skipReq >= Math.ceil((guilds[message.guild.id].voiceChannel.members.size - 1) / 2) || message.member.hasPermission("ADMINISTRATOR") || Admins[message.author.id+"-"+message.guild.id]) {
                  skip_song(message);
